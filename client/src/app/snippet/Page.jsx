...
import { addSnippets } from "../../redux/snippetSlice";
...
const SnippetsPage = () => {
  ...
  const savedSnippets = useSelector((state) => state?.snippet?.snippets);

  useEffect(() => {
    // If snippets are already in the Redux store, use them directly
    if (savedSnippets?.length > 0) {
      setSnippets(savedSnippets);
    }
  }, [savedSnippets, dispatch]);
  ...
  // Simulating fetching snippets from a Redux store or API
  if (savedSnippets?.length === 0) {
    // Snippets Skeleton
    <SnippetSkeleton />;
    // API call
